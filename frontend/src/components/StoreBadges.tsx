...
const APP_STORE_URL = 'https://apps.apple.com/app/voxtrade'
const GOOGLE_PLAY_URL = 'https://play.google.com/store/apps/details?id=com.voxtrade'

export const AppStoreBadge = ({ className = 'h-12 w-auto' }: { className?: string }) => (
  <a
    href={APP_STORE_URL}
    target="_blank"
...
export const GooglePlayBadge = ({ className = 'h-12 w-auto' }: { className?: string }) => (
  <a
...
const StoreBadges = ({ className = '', size = 'h-12 w-auto' }: { className?: string; size?: string }) => (
  <div className={`flex flex-wrap items-center gap-4 ${className}`}>
    <AppStoreBadge className={size} />
    <GooglePlayBadge className={size} />
  </div>
)
